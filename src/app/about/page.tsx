...
export const metadata = {
  ...
  description: "Learn more about igmrrf's background, technical stack, and engineering philosophy.",
};
    ...
    <div className="flex flex-col gap-24 max-w-4xl mx-auto">
      <div className="flex flex-col gap-6 border-l-4 border-primary pl-10 py-6 bg-accent/20">
        ...
        <p className="text-xl text-muted-foreground max-w-3xl leading-relaxed font-medium">
          Senior Full-Stack Engineer specializing in high-performance distributed systems 
      ...
      <div className="prose prose-slate dark:prose-invert max-w-none 
        ...
        <p>
          I believe that code should be more than just functional; it should be <strong>maintainable, 
          scalable, and business-centric</strong>. This is why I'm an advocate for <strong>Clean Architecture</strong> 
          and strict type systems. By decoupling business logic from infrastructure, we create systems 
          that can evolve with the needs of the organization without crumbling under technical debt.
        </p>

        <h2>Technical_Stack.json</h2>
        <p>
          With years of experience in the TypeScript ecosystem, I've built everything from complex 
          fintech integrations to high-performance TUI tools. I specialize in:
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 not-prose mt-8">
          {[
            { label: "Architecture", items: ["DDD", "Clean Architecture", "Microservices"] },
            { label: "Frontend", items: ["Next.js", "RSC", "Tailwind CSS", "Framer Motion"] },
            { label: "Backend", items: ["Node.js", "TypeScript", "PostgreSQL", "Prisma"] },
            { label: "AI Systems", items: ["RAG Pipelines", "LLM Orchestration", "Vector DBs"] },
          ].map((stack, i) => (
            <div key={i} className="p-6 border border-border bg-accent/20">
              ...
              <div className="flex flex-wrap gap-2">
                {stack.items.map(item => (
                  <span key={item} className="text-[10px] font-mono uppercase tracking-tighter text-muted-foreground">{item}</span>
                ))}
              </div>
            </div>
          ))}
        ...
        <p>
          When I'm not architecting systems, you can find me contributing to open-source, 
          building specialized Neovim plugins, or exploring the latest in terminal-based developer 
          productivity tools.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-px bg-border border border-border">
        {[
          ...
          { label: "LINK", icon: Linkedin, href: "https://linkedin.com/in/igmrrf" },
          ...
          >
            <social.icon className="h-4 w-4 group-hover:text-primary transition-colors" /> {social.label}
          </Link>
        ))}
      </div>
    </div>
