          ...
          <div className="flex flex-col gap-6 flex-1 p-12">
            <div className="flex items-center gap-6 text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground">
              <span className="flex items-center gap-2">
                <Calendar className="h-3 w-3" /> {study.date}
              </span>
              <span className="flex items-center gap-2">
                <Clock className="h-3 w-3" /> 10m_READ
              </span>
              <span className="text-primary font-black hidden sm:visible">
                ID: 00{idx + 1}
              </span>
            </div>
            <h2 className="text-3xl sm:text-4xl font-black group-hover:text-primary transition-colors uppercase italic tracking-tighter leading-none break-words line-clamp-2">
            ...
            <div className="flex flex-wrap gap-2 pt-4">
              {study.techStack.map((tag: string) => (
                <span
                  key={tag}
                  className="text-[10px] font-mono px-3 py-1 border border-border bg-background text-muted-foreground uppercase tracking-wider"
                >
                  {tag}
                </span>
              ))}
